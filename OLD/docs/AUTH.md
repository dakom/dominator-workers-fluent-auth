...
1. LoginTokenId (sent via http-only cookie, to prevent XSS attacks)
2. LoginTokenKey (sent via header, to prevent CSRF attacks)
...
AuthToken (durable object) - used for both LoginToken and OobTokens (forgot password, verify email, etc.)
- creation:
    - id is unique/random (a.k.a. LoginTokenId / OobTokenId)
    - self-delete time (configurable / provided)
    - stores:
        - key: generated random data (a.k.a. LoginTokenKey / OobTokenKey)
