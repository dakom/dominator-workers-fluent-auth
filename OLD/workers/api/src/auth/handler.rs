...
impl ApiBothWithExtraExt for AuthLogin {
    type Req = <AuthLogin as ApiBoth>::Req;
    type Res = <AuthLogin as ApiBoth>::Res;
    type Extra = AuthTokenCreateResponse;

    async fn handle(ctx: &ApiContext, data: AuthLoginRequest) -> ApiResult<(Self::Res, Self::Extra)> {
        async fn inner(ctx: &ApiContext, data: AuthLoginRequest) -> ApiResult<(AuthLoginResponse, AuthTokenCreateResponse)> {
            ...
            let uid = user.id;
            let auth_token = AuthTokenDO::create(&ctx.env, AuthTokenKind::Login, uid.clone(), user.user_token, AUTH_SIGNIN_TOKEN_EXPIRES).await?;
        ...
        inner(ctx, data).await.map_err(|_| {
            // can log the specific error here, but clients only see InvalidLogin
            // to avoid leaking semi-sensitive info (like who has an account etc.)
            AuthError::InvalidLogin.into()
        })
    }

    fn response(_ctx: &ApiContext, data: AuthLoginResponse, auth_token: AuthTokenCreateResponse) -> Response {
...
impl ApiBothWithExtraExt for AuthRegister {
    ...
    async fn handle(ctx: &ApiContext, data: AuthRegisterRequest) -> ApiResult<(Self::Res, Self::Extra)> {
        ...
        // sign the user in and return
        let auth_token = AuthTokenDO::create(&ctx.env, AuthTokenKind::Login, uid.clone(), user_token.clone(), AUTH_SIGNIN_TOKEN_EXPIRES).await?;
...
impl ApiBothWithExtraExt for AuthOpenIdFinalizeExec {
    ...
    async fn handle(ctx: &ApiContext, data: AuthOpenIdFinalizeRequest) -> ApiResult<(AuthOpenIdFinalizeExecResponse, AuthTokenCreateResponse)> {
        ...
        // sign the user in and return
        let auth_token = AuthTokenDO::create(&ctx.env, AuthTokenKind::Login, user.id.clone(), user.user_token.clone(), AUTH_SIGNIN_TOKEN_EXPIRES).await?;
        let auth_key = auth_token.key.clone();
...
impl ApiBothWithExtraExt for AuthConfirmResetPassword {
    ...
    async fn handle(ctx: &ApiContext, data: AuthConfirmResetPasswordRequest) -> ApiResult<(AuthConfirmResetPasswordResponse, AuthTokenCreateResponse)> {
        ...
        let auth_token = AuthTokenDO::create(&ctx.env, AuthTokenKind::Login, account.id.clone(), user_token.clone(), AUTH_SIGNIN_TOKEN_EXPIRES).await?;
        let auth_key = auth_token.key.clone();
