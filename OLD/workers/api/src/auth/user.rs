...
impl AuthUser {
    ...
    async fn validate(env: &Env, req: &Request, auth_kind: RouteAuthKind) -> ApiResult<AuthUser> {
        ...
        let AuthTokenValidateResponse {uid, user_token} = AuthTokenDO::validate(env, AuthTokenKind::Login, &token_id, token_key.clone(), AuthTokenAfterValidation::ExtendExpiresMs(AUTH_SIGNIN_TOKEN_EXPIRES)).await?;
