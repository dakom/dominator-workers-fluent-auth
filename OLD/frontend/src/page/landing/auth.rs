...
pub(super) async fn signin(email: &str, password: &str) -> ApiResult<()> {
    ...
    let AuthLoginResponse{uid, email_verified, auth_key} = AuthLogin::fetch(AuthLoginRequest { email: email.to_string(), password }).await?;
